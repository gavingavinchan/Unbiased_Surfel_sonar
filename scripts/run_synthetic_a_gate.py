#!/usr/bin/env python3
"""
Run the synthetic Dataset A gate end-to-end:
...
EVALUATOR_SCRIPT = REPO_ROOT / "scripts" / "eval_synthetic_sphere.py"
TRAIN_SCRIPT = REPO_ROOT / "debug_multiframe.py"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run synthetic Dataset A acceptance gate")
    parser.add_argument(
        "--dataset-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            "Generator pose mode. sonar_equivalent is the canonical Dataset A gate mode. "
    ...
    parser.add_argument("--run-prefix", type=str, default="debug_multiframe_synth")
...
def extract_eval_metrics(eval_json: Dict) -> Dict[str, float | bool]:
    gt = eval_json["ground_truth"]["radial_stats"]
    fit = eval_json.get("fit_sphere", {})
    ...
    return {
        ...
        "mean_radial_error_m": float(gt["mean"]),
        "p95_radial_error_m": float(gt["p95"]),
...
def write_summary_markdown(path: Path, summary: Dict) -> None:
    ...
    lines = [
        "# Synthetic Dataset A Gate Summary",
        "",
        ...
        f"- mean radial residual (m): `{gate.get('mean_radial_residual_m', float('inf')):.6f}`",
        f"- p95 radial residual (m): `{gate.get('p95_radial_residual_m', float('inf')):.6f}`",
    ...
    if run1 is None:
        ...
        lines.extend(
            [
                ...
                f"- run1 mean/p95/center (m): `{run1['metrics']['mean_radial_error_m']:.6f} / {run1['metrics']['p95_radial_error_m']:.6f} / {run1['metrics']['center_error_m']:.6f}`",
            ]
        )
    ...
    if run2 is not None:
        lines.extend(
            [
                ...
                f"- run2 mean/p95/center (m): `{run2['metrics']['mean_radial_error_m']:.6f} / {run2['metrics']['p95_radial_error_m']:.6f} / {run2['metrics']['center_error_m']:.6f}`",
                "",
                "## Reproducibility Drift",
                "",
                f"- pass: `{rep['pass']}`",
                f"- |delta mean| (m): `{rep['drift']['mean_radial_error_m']:.6f}` (<= `{rep['thresholds']['mean_radial_error_m_le']:.6f}`)",
                f"- |delta p95| (m): `{rep['drift']['p95_radial_error_m']:.6f}` (<= `{rep['thresholds']['p95_radial_error_m_le']:.6f}`)",
...
def main() -> None:
    ...
    summary = {
        ...
        "reproducibility": {
            "pass": False,
            "thresholds": {
                "mean_radial_error_m_le": float(args.drift_mean_threshold),
                "p95_radial_error_m_le": float(args.drift_p95_threshold),
                "center_error_m_le": float(args.drift_center_threshold),
            },
            "drift": {
                "mean_radial_error_m": float("inf"),
                "p95_radial_error_m": float("inf"),
    ...
    if not args.skip_training:
        ...
        for run_name in run_names:
            ...
            eval_json_path = eval_dir / "sphere_eval.json"
        ...
        drift = {
            "mean_radial_error_m": abs(float(run1["mean_radial_error_m"]) - float(run2["mean_radial_error_m"])),
            "p95_radial_error_m": abs(float(run1["p95_radial_error_m"]) - float(run2["p95_radial_error_m"])),
            "center_error_m": abs(float(run1["center_error_m"]) - float(run2["center_error_m"])),
        }

        thresholds = summary["reproducibility"]["thresholds"]
        reproducibility_pass = (
            drift["mean_radial_error_m"] <= float(thresholds["mean_radial_error_m_le"])
            and drift["p95_radial_error_m"] <= float(thresholds["p95_radial_error_m_le"])
    ...
    print("Synthetic Dataset A gate run complete")
